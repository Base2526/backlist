 ...
 * @SearchApiProcessor(
 *   id = "banlist_images",
 *   label = @Translation("Banlist - Images field"),
 *   description = @Translation("Add a Images field to search index"),
 *   stages = {
 *     "add_properties" = 0,
 *   },
 *   locked = true,
...
class BanlistImagesField extends ProcessorPluginBase {

  /**
   * machine name of the processor.
   * @var string
   */
  protected $processor_id = 'banlist_images_field';
  ...
  public function getPropertyDefinitions(DatasourceInterface $datasource = NULL) {
    ...
    if (!$datasource) {
      $definition = array(
        'label' => $this->t('Banlist - Images field'),
        'description' => $this->t('Banlist - Images field'),
        'type' => 'string',
    ...
    */
    ...
    if(!empty($node)){
      ...
      try {
        ...
        //       $images[] = trim( $field_bank_account[0]['value'] );
        //   } 
        // }
        ...
        foreach ($node->get('field_images')->getValue() as $imi=>$imv){
          ...
          // Utils::ImageStyle_BN($imv, 'bn_medium'))
          // Utils::ImageStyle_BN($imv, 'bn_thumbnail')

          // $images[] = array('fid'         => $imv['target_id'], 
          //                   'bn_medium'   => Utils::ImageStyle_BN($imv['target_id'], 'bn_medium'),
          //                   'bn_thumbnail'=> Utils::ImageStyle_BN($imv['target_id'], 'bn_thumbnail')
          //                   );
